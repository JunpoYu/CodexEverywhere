...
- **真实 Codex 会话**：创建、恢复和实时查看 app-server thread；以安全 Markdown 和 KaTeX 显示回复中的标题、列表、表格、代码与 LaTeX 公式，并结构化呈现计划、命令、文件修改、MCP、subagent、审批和错误。桌面端根据用户消息生成可收起的右侧大纲，窄屏通过侧滑面板快速定位；实时回复只在用户停留于底部时自动跟随，向上阅读历史后可通过“回到最新消息”恢复跟随。乐观发送、实时流和后台快照按 turn 与 item 身份归并，不会把同一条用户消息或首次回复重复显示。打开长会话时默认只读取最近 20 个 turn，用户可按需向前分页；后台仅修复最近 turn，收起的命令输出与文件差异按需生成 DOM，避免历史持续增长拖慢页面。旧版 Codex 不支持分页协议时会自动回退完整历史读取。
...
- **会话级权限与全局默认值**：对话页常驻显示 sandbox 与审批策略；统一设置中心允许用户把新会话默认 sandbox 和审批策略保存在自己的宿主机状态库中，换设备后仍保持一致。新建会话会明确显示并带入这些默认值，但已有会话继续使用 app-server 保存的自身权限，只有用户在 Web 或 TUI 中明确修改才会更新。
...
- **休眠与断网恢复**：Relay 会用 WebSocket ping/pong 淘汰半开 tunnel；页面从后台或断网状态恢复时先探测 Host，失效连接只重建和重新同步，不会自动重发可能已经执行的消息、审批或配置变更。

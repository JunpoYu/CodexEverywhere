...
> [!WARNING]
> 当前代码线为 `v0.4.0-alpha.18` 架构重建版。Gateway API v2 和新状态库不兼容 v0.3；v0.4 采用全新初始化，不迁移 v0.3 CE 状态。`v0.3.0-alpha.14` 是已完成实机验证的最后维护基线，只用于观察窗内恢复已保留的旧 CE 目录；已经运行 v0.4 的环境可将 `v0.4.0-alpha.17` 作为本次补丁升级的制品回退点。`~/.codex`、Codex 登录和 app-server 任务不属于清理范围。v0.4 Alpha tag/Prerelease 冻结待验收制品；当前 production 门槛先要求一个真实用户完成 staging。多用户并发、跨用户隔离和管理员控制面的实机验收延后，不阻塞当前单用户版本。
...
- 不提供 Web Terminal、第二套 HPC 调度器、第三方插件加载器或浏览器 `auth.json` 上传。
...
- 进入任务只加载最近 3 轮对话并自动定位到最新消息；每轮包含提问、回复和处理过程，工具调用不挤占对话名额。更早历史由用户按 3 轮显式加载，插入旧页时保持当前阅读锚点。同任务权威刷新会按稳定 item ID 更新最新窗口；未分页时同步推进游标，已分页时保留连续历史，避免跳过中间消息。旧 Agent 不支持按轮分页时退回原有的每页 50 项。
...
- 任务中心默认按更新时间汇总全部已授权工作区的任务，每张任务卡明确显示所属工作区，并可按工作区筛选；筛选范围同时作用于桌面端最近任务，但不会改变全局默认工作区。列表以 `thread/list` 建立权威快照，再增量合并 Agent 已知 lease 的状态变化；左侧运行状态无需轮询或重读历史即可更新，断线后仍由权威列表重建。同一筛选范围内刷新保留已有列表；重命名的多路通知合并为单飞请求及一次后续权威读取，失败时显示上次结果并可在侧栏重试。切换工作区或归档筛选时清空旧范围，避免混入其他范围的任务。
- 新建任务会在第一轮发送前从当前 app-server 动态读取可用模型及其推理强度，不维护模型白名单；进入新任务页或任务设置、完成 Codex 登录、更新或手动重启 app-server 后都会重新读取目录，因此账号获得新模型后无需升级 CE 前端。页面同时显示当前全局 Sandbox 与审批默认值；模型、推理强度和单次权限覆盖通过同一个 `thread/start` 边界提交。模型目录不可用时仍可选择 Codex 默认值。未覆盖的权限字段由 Agent 在 Codex 接受创建时从同一 revision 的全局偏好解析；全局偏好更新与该创建边界共用短期协调锁，避免其他设备的并发修改被旧页面冻结或覆盖。
- 主时间线默认直接呈现用户消息、Codex 最终回答、计划与错误；同一 turn 的 commentary、推理摘要、命令、文件修改、MCP、subagent 和未知事件收进默认折叠的“处理过程”，需要时仍可逐项展开。旧模型未提供 message phase 时，只在 turn 已结束后把最后一条 assistant 消息作为兼容最终回答；折叠内容在打开前不挂载大型 DOM。
...
- 对话页不使用常驻右侧属性栏；模型、推理强度、权限、app-server 实时报告的上下文窗口占用与权威历史中的上下文压缩次数，作为“发送前检查”显示在桌面端输入框左侧，空间不足时收拢到输入框上方。手机端默认只显示一行模型和上下文占用，点按展开完整摘要，设置按钮仍可直接打开；输入框默认紧凑，聚焦后增高，旁支入口与发送按钮共用操作栏。上下文达到 70%/90% 时分别显示警告/危险状态，高权限组合也会被显式标记。待处理交互就近显示在输入区上方；当前任务的 Queue 数量与 app-server 恢复出的实际工作目录显示在输入区下方，长路径在自身区域内横向滚动且不会撑宽页面。
...
- 主任务完成至少一轮后，在输入框发送 `/side` 或 `/side 问题` 打开旁支。桌面按需并列显示，窄屏切为单栏；主任务可继续运行，旁支显示其状态。
- 每个主任务最多一个未结束旁支，不支持嵌套。上下文固定继承至创建时最近完成的轮次，后续主任务进展不会自动加入；界面只展示旁支的新消息，历史仍由 app-server 分页读取。
...
- 需要支持 `thread/fork.lastTurnId` 和 `thread/turns/list` 的 Codex（契约测试基线 0.154.0）。不兼容时明确失败，不退回旧版 ephemeral/continuity buffer。创建或删除结果无法证明时保留待核对状态，不自动重复副作用。创建或删除结果未知时，用户可明确确认可能存在遗留会话后解除旁支占用；这只移除 CE 关联，遗留 Codex 会话或运行任务须在任务列表或宿主机核对，不宣称已删除。
...
| 旧 Side 协议、通用 `thread/fork`、浏览器 `auth.json` 导入 | 已移除     |

...
> [!WARNING]
> 当前代码线为 `v0.4.0-alpha.15` 架构重建版。Gateway API v2 和新状态库不兼容 v0.3；v0.4 采用全新初始化，不迁移 v0.3 CE 状态。`v0.3.0-alpha.14` 是已完成实机验证的最后维护基线，只用于观察窗内恢复已保留的旧 CE 目录；已经运行 v0.4 的环境可将 `v0.4.0-alpha.14` 作为本次补丁升级的制品回退点。`~/.codex`、Codex 登录和 app-server 任务不属于清理范围。v0.4 Alpha tag/Prerelease 冻结待验收制品；当前 production 门槛先要求一个真实用户完成 staging。多用户并发、跨用户隔离和管理员控制面的实机验收延后，不阻塞当前单用户版本。
...
- 对话页不使用常驻右侧属性栏；模型、推理强度与权限作为“发送前检查”显示在桌面端输入框左侧，空间不足或移动端时收拢到输入框上方，高权限组合会被显式标记。待处理交互就近显示在输入区上方；当前任务的 Queue 数量与 app-server 恢复出的实际工作目录显示在输入区下方，长路径在自身区域内横向滚动且不会撑宽页面。

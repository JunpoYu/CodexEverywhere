...
- 对话页不使用常驻右侧属性栏；模型、推理强度、权限、app-server 实时报告的上下文窗口占用与权威历史中的上下文压缩次数，作为“发送前检查”显示在桌面端输入框左侧，空间不足或移动端时收拢到输入框上方。上下文达到 70%/90% 时分别显示警告/危险状态，高权限组合也会被显式标记。待处理交互就近显示在输入区上方；当前任务的 Queue 数量与 app-server 恢复出的实际工作目录显示在输入区下方，长路径在自身区域内横向滚动且不会撑宽页面。

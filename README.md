...
- 输入 `/side <问题>` 可基于当前上下文开启真正的 Codex ephemeral fork。Web 会用独立的 Side 标识和返回入口隔离这条临时支线；Agent 只返回版本化的继承边界而不把完整父历史再次传给浏览器，并在协议入口拒绝向 Side 写入持久 Queue。创建连接中断时 Web 保留原幂等键继续确认，Side 内仍有待确认消息时禁止离开；它不进入会话列表，刷新或离开后不可恢复，回复也不会自动合并回主会话。

...
- 每条首次认证或静默恢复产生的新物理连接，都会在接管页面状态前重新协商 Side 事件 ACK；协商失败会关闭尚未绑定的连接，不会让恢复后的页面停止确认事件或泄漏会话。静默恢复票据仍有效时会继续自动重试，只有 Host 明确要求重新认证才切换到 Passkey 登录。
...
- Codex app-server 当前不允许对 ephemeral thread 使用 `includeTurns`、`thread/turns/list`，并可能因为没有磁盘 rollout 拒绝 `thread/resume`。因此 Agent 会让持有 Side 的页面认证票据继续持有原 app-server 客户端：有效 silent resume 会在旧 WebSocket 报告 close 之前先夺取事件所有权，Side 的实时订阅不断开。新版 Agent 通过 `side-continuity-ack-v1` capability 让新版 Web 显式启用确认模式；启用后事件由 Agent 有界缓存并在新握手后按序交付，只有浏览器处理并收到 version 1 ACK 结果后才删除，确认帧丢失时按稳定 event ID 去重重放。缓存中的旧 PWA 不会发送 enable，继续采用原有的交付即消费兼容模式，不会被新版 ACK 规则锁死。若确认模式下 4096 条或 16 MiB 安全上限仍被耗尽，Agent 发送版本化 gap，Web 停止该 Side 并返回主会话人工核对，绝不静默丢弃后继续显示不完整支线。Web 正常刷新或关闭时发送版本化释放请求；纯网络断线或页面进入后台不会释放，任一活动 transport 也永不因空闲超时。若浏览器被系统直接终止而未送达释放请求，最后一个 transport 消失后仍保留 24 小时恢复宽限，期间成功重连会取消回收计时；Side 一旦被归类进入该宽限，后续 app-server socket 提前关闭也不会取消 ticket 回收，超过宽限便关闭无人能够继续持有 token 的 continuity 和缓冲。Web 仍只用 `thread/read(includeTurns:false)` 核对状态，并以当前页面已经收到的有界 fork/完成事件快照恢复视图；不会用空 read 覆盖已有内容，也不会把 `no rollout found` 误判为 Side 已消失。若页面票据、Agent 或 app-server 本身已经失效，则无法补回此前未收到的 ephemeral 历史。

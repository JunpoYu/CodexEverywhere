...
> [!WARNING]
> CodexEverywhere 目前处于 Alpha 阶段，最新预发布版本为 `v0.3.0-alpha.8`。协议、配置和存储结构仍可能变化，现阶段建议用于个人环境或可信团队试用。生产部署请只使用经过 CI 验证的 [GitHub Release](https://github.com/JunpoYu/CodexEverywhere/releases) 制品。
...
- 输入 `/side <问题>` 可基于当前上下文开启真正的 Codex ephemeral fork。Web 会用独立的 Side 标识和返回入口隔离这条临时支线；Agent 只返回版本化的继承边界而不把完整父历史再次传给浏览器，并在协议入口拒绝向 Side 写入持久 Queue。创建连接中断时 Web 保留原幂等键继续确认，并在返回缓存结果前验证临时 thread 仍存在；同宿主机重新认证会恢复 Side 视图，Side 内仍有待确认消息时禁止离开。它不进入会话列表，刷新、app-server 重启或主动离开后不可恢复，回复也不会自动合并回主会话。

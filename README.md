...
- 需要支持 `thread/fork.lastTurnId` 和 `thread/turns/list` 的 Codex（契约测试基线 0.154.0）。不兼容时明确失败，不退回旧版 ephemeral/continuity buffer。创建或删除结果无法证明时保留待核对状态，不自动重复副作用。创建结果未知且缺少会话 ID 时，用户可明确确认可能存在遗留会话后解除旁支占用；这只移除 CE 关联，遗留 Codex 会话须在宿主机核对，不宣称已删除。

...
- 配对、Passkey、密码或恢复认证已经成功但首次 ACK enable 暂时失败时，Web 会使用该新 client 刚签发的内存恢复票据无限退避重连，不要求用户重复认证。Side continuity overflow 在活动 turn 中发生时先请求 `turn/interrupt`，只有确认停止后才返回父会话；停止失败则保留 Side 控制界面。retained app-server inner 意外关闭时，Agent 也会发送带 `inner-closed` 原因的 version 1 continuity gap 并等待确认，不再因 app-server 实例 ID 未变化而把缺失事件伪装成可连续恢复。

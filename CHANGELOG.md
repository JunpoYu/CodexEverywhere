...
## [0.4.0-alpha.18] - 2026-09-14

- 手机端任务配置默认收为单行模型、上下文占用和运行状态，可展开详情或直接打开设置。
- 压缩手机输入区高度，旁支入口并入发送操作栏，保留 44px 触控目标。
- 编辑草稿时保留输入框高度，避免点击发送时失焦收缩导致按钮移动。
- 用户数据库保持 schema 2；从 alpha.17 升级无需数据库迁移，可回退 alpha.17 制品。

## [0.4.0-alpha.17] - 2026-09-10

### Added

- 新增 `/side` 旁支问答：桌面侧栏、移动端单列视图，支持收起、带回主对话草稿与明确结束删除；通过原生 app-server fork 固定继承边界，默认只读执行权限。

### Fixed

- 最近历史按完整 turn 加载，修复只显示最新回复片段与加载更早消息后未正确显示的问题。
- 重命名后的任务列表刷新保留已有结果、合并重复请求，并在失败时提供重试入口，避免侧栏偶发空白。
- 时间线布局修正使用即时滚动，避免平滑滚动干扰历史锚点与跟随状态。

### Changed

- 使用 Codex CLI `0.154.0` 重新生成 app-server 协议基线并验证兼容性。
- 用户 CE 状态库从 schema 1 原子升级到 schema 2，新增旁支元数据；管理员状态库保持 schema 1。部署前必须备份用户 CE 数据库，回退旧版本必须恢复升级前数据库；不回滚 `~/.codex`。
...
- 左侧最近任务和任务中心会把 Agent 已知 lease 的 `thread/state` 增量合并到 `thread/list` 权威快照；运行、等待操作、完成和失败会同步更新，不再长期停留在旧的灰色空闲点。初始列表会覆盖当前 lease 状态，替换 lease 的首次权威读取也会发布状态；状态事件不会触发列表轮询或重复读取，并且列表请求期间到达的新状态不会被较早的响应或失败分页留下的旧缓冲覆盖。
- Agent 重启、lease 失败或一次权威 `thread/open` 明确没有用量投影时会清除浏览器中的旧上下文占用；若新用量事件恰好在打开请求期间到达，则仍保留较新的事件值，避免被较早的响应回退。

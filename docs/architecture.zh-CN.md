...
首次配对、Passkey、密码或恢复认证成功后签发的内存票据同样参与上述重试：首次 enable 失败会关闭未绑定 transport，再用该票据进行非交互 resume，不丢弃已完成的身份验证。continuity overflow 若发生在活动 Side turn，Web 必须先用仍可用的控制通道请求 `turn/interrupt`；只有收到成功响应才清除 Side 并返回父会话，失败或结果未知时保留 Side 控制界面供用户重试。retained app-server inner 的本地 socket 意外关闭时，即使 app-server 进程身份未变，Agent 也会把现有缓冲替换为 `reason: "inner-closed"` 的 version 1 gap；新版 Web 按 overflow 失败关闭流程处理并确认该 gap，不能把无法从 ephemeral rollout 补回的时间窗当成正常连续恢复。
...
- PWA 通过 `thread/read` 快照和增量 notification 结构化显示用户消息、Codex 回复、plan、命令、文件修改、MCP、subagent、错误和未知 generic event。打开会话默认只渲染最近 20 个 turn，更早历史由用户显式分页加载；分页初始化失败后必须重试 `thread/resume`，初始化重试不受普通 repair 的实时静默门槛或当前 turn 是否仍 active 限制，任何完成路径也不得停止其计时器。新会话从首个 turn 起即使用分页同步，并可从 repair 首次取得更早历史 cursor；用户加载到历史尽头后记录 exhaustion 及当时的最新 turn 边界，repair 页仍覆盖该边界时保持耗尽状态，边界被更多新 turn 推出窗口时则重新建立 cursor，避免漏失中间 turn。若旧 app-server 不支持 `thread/turns/list`，新会话会降级到 legacy repair；完整结果进入 Timeline 前仍裁剪到默认窗口，但未截断 turn 仅用于待确认 mutation 的权威对账。每张时间线卡片显示相对时间并以本地精确时间作为辅助信息；快照优先使用 turn 起止时间，实时卡片使用 item 生命周期时间，缺失旧数据时从 UUIDv7 turn id 恢复近似创建时间。reasoning item 及其增量事件不进入 Web 时间线，Web 也不覆盖 Codex 的 reasoning summary 配置。
...
- Ephemeral Side 是普通历史同步规则的明确例外：当前 app-server 对它拒绝 `thread/read(includeTurns:true)` 和 `thread/turns/list`，且内存 thread 存在时仍可能因没有 rollout 拒绝 `thread/resume`。认证恢复票据因此持有一个可引用计数的 app-server 会话连续体；只要该会话包含 ephemeral thread，WebSocket 关闭只释放当前 Gateway transport 引用，不关闭 app-server 客户端。有效 silent resume 在握手完成前即生成新的独占 handle、清除旧 handle 的 listener，并拒绝旧 handle 后续请求；所以即使旧 socket 半开，事件也只进入连续体缓冲或当前 transport。ACK 不是 `side-fork-v1` 的隐式升级：Agent 另行声明 `side-continuity-ack-v1`，新版 Web 认证后发送 version 1 `auth/session/events/enable` 并验证 version 1 结果，只有成功协商的 continuity 才按到达顺序保留尚未确认的事件；浏览器以稳定 event ID 去重并发送 version 1 批量 ACK，Agent 返回 version 1 结果并在确认后删除前缀。缓存中的旧 Web 不发送 enable，Agent 保持旧版“空窗有界缓存、listener 注册即消费”的语义。确认模式的未确认事件最多 4096 个、合计 16 MiB；触顶时不删除最旧事件，而是将缓冲替换为版本化 continuity gap 并停止普通事件交付，Web 收到后失败关闭当前 Side、把待确认操作转为人工核对并返回父 thread。没有 ephemeral thread 的普通页面断线后立即释放 inner client；异步创建的普通 inner 即使晚于 transport 关闭才完成，也会重新执行生命周期判断并立即释放。页面 `pagehide` 且未进入 bfcache 时，在清除浏览器内存 token 前同步排入 version 1 加密 `auth/session/release` 请求，响应同样携带 version 1；正常刷新和关闭因此释放 ticket，网络断线或后台冻结则保留 ticket。活动 transport 没有空闲 TTL；最后一个 transport 关闭而 continuity 已经持有 ephemeral Side 时将该对象持久归类为待回收并启动 24 小时断连宽限，任何成功 resume 会取消 timer，之后再次断开才重新计时，但分类不会因 retained app-server socket 随后关闭而丢失。宽限届满仍无 transport 时删除关联 ticket 与缓冲并关闭仍存在的 inner，使未触发 `pagehide` 或释放帧丢失的手机页面不会永久占用资源；这是异常终止回收边界，意味着完全离线超过 24 小时的页面不能继续原 ephemeral Side。票据撤销、显式释放或淘汰且不存在活动 transport 时也关闭保留客户端；每设备和全局 ticket LRU 继续作为硬上限。Web 只把 `thread/read(includeTurns:false)` 用作存在性和状态检查，最近 20 个 Side turn 来自当前页面持有的有界 fork 快照与后续完整 `turn/completed` 事件；修复读取不得以空 turns 覆盖这份快照，也不得用 `no rollout found` 推断 Side 消失。页面票据、Agent 或 app-server 已失效时仍无法补回此前未收到的 ephemeral 历史。
...
- Web 为刚发送的用户消息创建乐观卡片，并只以稳定的 item ID 或 `clientUserMessageId` 精确绑定身份；同 turn 内其他用户消息不能作为 Steer 新消息已经同步的证据。assistant 流式卡片除 item id 外还记录 turn id、消息类型与累计原文；只有 item 完成或完成快照才能在生命周期阶段按同一 turn 的类型和完整文本唯一匹配不同 item id，`item/started` 只能建立自身生命周期身份；若权威 item 的精确 ID 已存在，则禁止再跨 ID 删除任何同文 sibling stream，多个候选时同样禁止猜测。权威 turn 已结束时快照必须替换流式卡片，`turn/completed` 还会清理同 turn 的孤儿流式卡；repair 窗口未覆盖的旧流式卡保留已有内容、原时间线位置并结束动画，同时保留可供迟到完成事件或历史分页精确匹配的 turn、类型与原文身份，权威页到达后必须清理 loose 副本。

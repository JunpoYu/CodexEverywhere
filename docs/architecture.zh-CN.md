...
Agent composition root 只构造服务、绑定 Scope、连接控制面事件并完成静态装配，不直接声明业务 handler。用户 Gateway 方法在独立的 core handler registry 中按 host、workspace、model、thread、Queue 和 preferences 分组；Identity 与 Setup 使用各自的 handler map。`ModelCatalogService` 只负责 app-server 目录的运行时校验与稳定字段投影。`SetupService` 把 Codex 更新和 app-server 切换视为一个完整生命周期：第一次安全检查先避免已知忙碌时开始安装；二进制安装后，`CodexRuntimeGate` 复用状态库跨进程 coordination lock，将最终权威检查和重启放在同一栅栏内。`ThreadService`、Queue dispatcher 和 `ce tui` proxy 的 `thread/start`、`turn/start`、`turn/steer` 在 app-server 接受请求前持有同一把锁，因此不能插入最终检查与关闭之间。安装期间允许继续使用 Codex；若产生新活动任务，最终检查失败并保留待切换状态，不中断该任务。权威 `thread/list` 分页仍覆盖 Web、TUI、Exec、IDE 和 subagent 来源，只有不存在活动任务且 app-server 状态可验证时才允许重启。直接连接 app-server 而不经过 CE 的 IDE/CLI 不可能取得这把 CE 锁，属于明确的外部协调边界，操作者必须在更新前停止；CE 不声称能够冻结任意第三方 client。安装开始前先原子写入 `0600` 的最小运行时切换状态，安装完成后记录 `restart-required`；只有 app-server 健康切换后才清除，重连浏览器可读取该状态继续恢复。该文件不含 prompt、路径、凭据或 Queue 内容。`completed` 事件只在上述边界全部成功后发出。`ThreadService` 是 thread 用例 facade，只编排 client、lease、workspace 和偏好锁；`ThreadSessionCoordinator` 独占每任务的权限 coordination fence、每 lease 一次 resume 和运行设置缓存。`AutoTitleService` 只接收已被 Codex 接受的 user message，在对应 turn 成功后用纯本地规则产生有界标题，并通过 `thread/name/set` 写回 app-server；它不调用模型、不持久化 prompt、不维护第二份 thread 名称。Codex JSON 运行时边界、权威 thread 到 CE timeline/summary 的纯投影、以及 thread settings 的双向转换分别位于独立模块。投影和设置转换模块不得保存会话状态或发起 app-server 请求，session coordinator 不得反向依赖 Gateway、Queue 或展示投影。

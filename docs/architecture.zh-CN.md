...
- Ephemeral Side 是普通历史同步规则的明确例外：当前 app-server 对它拒绝 `thread/read(includeTurns:true)` 和 `thread/turns/list`，且内存 thread 存在时仍可能因没有 rollout 拒绝 `thread/resume`。认证恢复票据因此持有一个可引用计数的 app-server 会话连续体；只要该会话包含 ephemeral thread，WebSocket 关闭只释放当前 Gateway transport 引用，不关闭 app-server 客户端。有效 silent resume 在握手完成前即生成新的独占 handle、清除旧 handle 的 listener，并拒绝旧 handle 后续请求；所以即使旧 socket 半开，事件也只进入连续体缓冲或当前 transport。连续体按到达顺序保留尚未由浏览器确认的事件，Direct/Relay 的 Noise 握手回复先发送，新 transport 再订阅回放；浏览器以稳定 event ID 去重并批量确认，只有确认后 Agent 才删除前缀，因此确认帧丢失不会形成事件空洞。未确认事件最多 4096 个、合计 16 MiB；触顶时不删除最旧事件，而是将缓冲替换为版本化 continuity gap 并停止普通事件交付，Web 收到后失败关闭当前 Side、把待确认操作转为人工核对并返回父 thread。没有 ephemeral thread 的普通页面断线后立即释放 inner client；异步创建的普通 inner 即使晚于 transport 关闭才完成，也会重新执行生命周期判断并立即释放。页面 `pagehide` 且未进入 bfcache 时，在清除浏览器内存 token 前同步排入 version 1 加密 `auth/session/release` 请求，响应同样携带 version 1；正常刷新和关闭因此释放 ticket，网络断线或后台冻结则保留 ticket。活动 transport 没有空闲 TTL；最后一个 transport 关闭而 continuity 仍持有 ephemeral Side 时启动 24 小时断连宽限，任何成功 resume 会取消该 timer，之后再次断开才重新计时。宽限届满仍无 transport 时删除关联 ticket 并关闭 inner，使未触发 `pagehide` 或释放帧丢失的手机页面不会永久占用资源；这是异常终止回收边界，意味着完全离线超过 24 小时的页面不能继续原 ephemeral Side。票据撤销、显式释放或淘汰且不存在活动 transport 时也关闭保留客户端；每设备和全局 ticket LRU 继续作为硬上限。Web 只把 `thread/read(includeTurns:false)` 用作存在性和状态检查，最近 20 个 Side turn 来自当前页面持有的有界 fork 快照与后续完整 `turn/completed` 事件；修复读取不得以空 turns 覆盖这份快照，也不得用 `no rollout found` 推断 Side 消失。页面票据、Agent 或 app-server 已失效时仍无法补回此前未收到的 ephemeral 历史。

...
- Ephemeral Side 是普通历史同步规则的明确例外：app-server 对它可能拒绝完整历史读取与 `thread/resume`，所以认证恢复票据持有可引用计数的原 app-server 会话连续体。silent resume 会先独占事件 handle，再让旧 transport 退出。`side-continuity-ack-v1` 逐物理连接启用；浏览器按稳定 event ID 去重并累计 ACK，Agent 只缓存属于当前 ephemeral thread 的事件，其他 thread、Queue 与宿主机事件断线时直接跳过。未确认 Side 事件最多 4096 个、合计 16 MiB；触顶或 inner 关闭会产生终态 gap。gap ACK 只清除重放标记，普通请求仍被拒绝，Web 必须保持仅控制 UI；只有确认停止活动 turn 并收到 version 1 `side/session/release` 成功，才清理 Side 和权限缓存并返回父 thread。旧 Web 没有 ACK 能力时仍采用消费式交付，但不能绕过终态 gap。普通页面断线后立即释放 inner；Side 异常断连最多保留 24 小时。Web 只用 `thread/read(includeTurns:false)` 核对状态；重连快照以 merge 方式补入完成 turn，不得替换 DOM 中已 ACK 的流式内容。实例代次明确改变时，旧 ephemeral Side 已确定消失，可直接清理本地控制入口；其他读取失败不得推断消失。

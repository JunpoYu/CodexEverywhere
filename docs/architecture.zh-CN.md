...
当已接受的 Web 或 Queue 消息具有可用标题且 app-server thread 尚无明确名称时，`AutoTitleService` 在 turn 完成前持有一个短期 effect 引用。只有收到相同 turn ID 的成功完成事件后，它才重新读取权威 `thread.name` 并在仍为空时调用 `thread/name/set`。lease 按 turn ID 暂存尚未观察到对应 `turn/start` 响应的终态；响应到达后，同步完成可选 effect 的注册，再由 Scope timer 释放该项。这样既覆盖多个极短 turn 的乱序响应，又不缓存正文或重建连续事件 buffer。手动 Web 重命名会先取消待处理自动命名；TUI 或其他 client 的 `thread/name/updated` 在自动写入期间也持续被观察，若与自动标题冲突则最终恢复明确名称。成功的 service-owned 重命名只通过版本化 `thread/name/changed` 控制事件通知所有已认证任务列表重新读取权威摘要，事件不携带标题内容。通用短指令、失败或中断的 turn、已有名称以及任何自动命名错误都不得改写名称或改变原 mutation 结果；effect 释放后 lease 仍按正常 idle 规则关闭。

...
- Ephemeral Side 是普通历史同步规则的明确例外：当前 app-server 对它拒绝 `thread/read(includeTurns:true)` 和 `thread/turns/list`，且内存 thread 存在时仍可能因没有 rollout 拒绝 `thread/resume`。Web 只把 `thread/read(includeTurns:false)` 用作存在性和状态检查，最近 20 个 Side turn 来自当前页面持有的有界 fork 快照与后续完整 `turn/completed` 事件；修复读取不得以空 turns 覆盖这份快照，也不得用 `no rollout found` 推断 Side 消失。这里的“重开/恢复视图”仅指同一页面已观察到的快照，不承诺补回 transport gap 中未收到的 ephemeral 历史。

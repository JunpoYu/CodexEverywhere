...
- PWA 通过 `thread/read` 快照和增量 notification 结构化显示用户消息、Codex 回复、plan、命令、文件修改、MCP、subagent、错误和未知 generic event。打开会话默认只渲染最近 20 个 turn，更早历史由用户显式分页加载；分页初始化失败后必须重试 `thread/resume`，初始化重试不受普通 repair 的实时静默门槛或当前 turn 是否仍 active 限制，不得让 `initializing` 成为终态。新会话从首个 turn 起即使用分页同步，并可从 repair 首次取得更早历史 cursor；用户加载到历史尽头后单独记录 exhaustion，后续 repair 不得重新激活已耗尽的 cursor。若旧 app-server 不支持 `thread/turns/list`，新会话会降级到 legacy repair；完整结果进入 Timeline 前仍裁剪到默认窗口，但未截断 turn 仅用于待确认 mutation 的权威对账。每张时间线卡片显示相对时间并以本地精确时间作为辅助信息；快照优先使用 turn 起止时间，实时卡片使用 item 生命周期时间，缺失旧数据时从 UUIDv7 turn id 恢复近似创建时间。reasoning item 及其增量事件不进入 Web 时间线，Web 也不覆盖 Codex 的 reasoning summary 配置。
...
- Web 为刚发送的用户消息创建乐观卡片，并只以稳定的 item ID 或 `clientUserMessageId` 精确绑定身份；同 turn 内其他用户消息不能作为 Steer 新消息已经同步的证据。assistant 流式卡片除 item id 外还记录 turn id、消息类型与累计原文；只有 item 完成或完成快照才能在生命周期阶段按同一 turn 的类型和完整文本唯一匹配不同 item id，`item/started` 只能建立自身生命周期身份；若权威 item 的精确 ID 已存在，则禁止再跨 ID 删除任何同文 sibling stream，多个候选时同样禁止猜测。权威 turn 已结束时快照必须替换流式卡片，`turn/completed` 还会清理同 turn 的孤儿流式卡；repair 窗口未覆盖的旧流式卡保留已有内容、原时间线位置并结束动画，不得在会话空闲后继续显示加载光标或被移动到最新位置。

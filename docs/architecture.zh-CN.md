...
- Ephemeral Side 是普通历史同步规则的明确例外：当前 app-server 对它拒绝 `thread/read(includeTurns:true)` 和 `thread/turns/list`，且内存 thread 存在时仍可能因没有 rollout 拒绝 `thread/resume`。认证恢复票据因此持有一个可引用计数的 app-server 会话连续体；只要该会话包含 ephemeral thread，WebSocket 关闭只释放当前 Gateway transport 引用，不关闭 app-server 客户端，随后以同一页面票据静默恢复的 transport 复用该客户端和既有订阅。两个 transport 之间没有 listener 时，连续体按到达顺序保留最近 4096 个、合计至多 16 MiB 的事件；Direct/Relay 的 Noise 握手回复先发送，新 transport 再订阅并同步回放，避免把加密事件排在握手前。没有 ephemeral thread 的普通页面断线后立即释放 inner client；票据撤销或淘汰且不存在活动 transport 时也关闭保留客户端。Web 只把 `thread/read(includeTurns:false)` 用作存在性和状态检查，最近 20 个 Side turn 来自当前页面持有的有界 fork 快照与后续完整 `turn/completed` 事件；修复读取不得以空 turns 覆盖这份快照，也不得用 `no rollout found` 推断 Side 消失。页面票据、Agent 或 app-server 已失效时仍无法补回此前未收到的 ephemeral 历史。

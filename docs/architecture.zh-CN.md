...
Side 连续体的 ACK 是逐物理连接协商的能力：首次认证和 silent resume 都必须在 Web 接管活动状态前完成 enable；协商失败时关闭尚未绑定的 transport。silent resume 的可复用票据仍有效时，协商错误作为同一次连接恢复尝试失败进入无限退避，不触发 WebAuthn；只有明确的 `REAUTH_REQUIRED` 才转入交互认证。启用后的累计 event ACK 保留最新待确认 ID，失败时以 50 ms 起步、5 秒封顶的指数退避持续重试；transport 失效则清理 timer，由新连接重新协商。ACK 只证明浏览器处理了连续前缀，不能清除已经发生的 gap。旧 Web 没有 ACK 能力时保持消费式交付，但 gap 后同样由 Agent 失败关闭普通请求。

首次配对、Passkey、密码或恢复认证成功后签发的内存票据同样参与上述重试：首次 enable 失败会关闭未绑定 transport，再用该票据进行非交互 resume，不丢弃已完成的身份验证。多个激活流程由单调 epoch 仲裁，旧登录或旧恢复在每个异步边界后都必须确认自己仍是当前流程。continuity overflow 若发生在活动 Side turn，Web 必须立即切为仅控制状态，再用仍可用的控制通道请求 `turn/interrupt`；只有收到成功响应并完成 version 1 `side/session/release` 才清除 Side，失败或结果未知时保留 Side 控制界面。retained app-server inner 的本地 socket 意外关闭时，即使 app-server 进程身份未变，Agent 也会把现有缓冲替换为 `reason: "inner-closed"` 的 version 1 gap；确认 gap 不会恢复普通事件或 mutation。
...
- 输入器不复制 TUI 的完整斜杠指令清单，也不开放任意 Shell adapter。当前唯一原生 Web 指令是 `/side <问题>`：Web 只有同时协商到 `side-fork-v1` 与 `side-session-control-v1` 才允许 mutation，重连后也必须对新 client 重新验证。Agent 通过 app-server `thread/fork` 创建 `ephemeral: true` 的真实临时 fork，响应裁掉继承 rollout，只携带版本化继承边界与不透明 app-server 实例代次。继承边界参与 durable identity，缓存结果返回前重新验证 ephemeral thread 仍存在。Web 只渲染 fork 后新增 turn；分叉结果未知时保留原幂等键，不生成第二个 fork。Side 不进入普通 thread 列表，不支持持久 Queue，也不会自动合并回父 thread。父 thread、Side 活动 turn、Side 待确认消息或未确认 release 都会阻止切换；其他 `/` 输入失败关闭。
- Ephemeral Side 是普通历史同步规则的明确例外：app-server 对它可能拒绝完整历史读取与 `thread/resume`，所以认证恢复票据持有可引用计数的原 app-server 会话连续体。silent resume 会先独占事件 handle，再让旧 transport 退出。`side-continuity-ack-v1` 逐物理连接启用；浏览器按稳定 event ID 去重并累计 ACK。明确属于其他 thread 或 Queue 的事件断线时直接跳过；无法提取 thread 身份的 `codex/*` 新事件按向前兼容原则保守缓存。每条 continuity 最多保留 4096 个未确认事件、16 MiB；每用户 Agent 另有共享 32 MiB 事件预算和 32 个 retained Side client 名额，`thread/fork` 与 durable Side 恢复必须在接触 app-server 前取得名额。单条或共享预算触顶、或 inner 关闭会产生终态 gap。gap ACK 只清除重放标记，普通请求仍被拒绝，Web 必须保持仅控制 UI；只有确认停止活动 turn 并收到 version 1 `side/session/release` 成功，才清理 Side、权限缓存和共享预算并返回父 thread。旧 Web 没有 ACK 能力时仍采用消费式交付，但不能绕过终态 gap。普通页面断线后立即释放 inner；Side 异常断连最多保留 24 小时。Web 只用 `thread/read(includeTurns:false)` 核对状态；重连快照以 merge 方式补入完成 turn，不得替换 DOM 中已 ACK 的流式内容。实例代次明确改变时，旧 ephemeral Side 已确定消失，可直接清理本地控制入口；其他读取失败不得推断消失。

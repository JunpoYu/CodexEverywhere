...
- Ephemeral Side 是普通历史同步规则的明确例外：当前 app-server 对它拒绝 `thread/read(includeTurns:true)` 和 `thread/turns/list`，且内存 thread 存在时仍可能因没有 rollout 拒绝 `thread/resume`。认证恢复票据因此持有一个可引用计数的 app-server 会话连续体；只要该会话包含 ephemeral thread，WebSocket 关闭只释放当前 Gateway transport 引用，不关闭 app-server 客户端。有效 silent resume 在握手完成前即生成新的独占 handle、清除旧 handle 的 listener，并拒绝旧 handle 后续请求；所以即使旧 socket 半开，事件也会进入连续体缓冲而不是只发往旧 transport。两个 transport 之间没有 listener 时，连续体按到达顺序保留最近 4096 个、合计至多 16 MiB 的事件；Direct/Relay 的 Noise 握手回复先发送，新 transport 再订阅并同步回放，避免把加密事件排在握手前。没有 ephemeral thread 的普通页面断线后立即释放 inner client；页面 `pagehide` 且未进入 bfcache 时，在清除浏览器内存 token 前同步排入加密 `auth/session/release` 请求，正常刷新和关闭因此释放 ticket，网络断线或后台冻结则保留 ticket 并继续无限恢复。票据撤销、显式释放或淘汰且不存在活动 transport 时关闭保留客户端；异常崩溃仍由每设备和全局 ticket LRU 硬上限兜底。Web 只把 `thread/read(includeTurns:false)` 用作存在性和状态检查，最近 20 个 Side turn 来自当前页面持有的有界 fork 快照与后续完整 `turn/completed` 事件；修复读取不得以空 turns 覆盖这份快照，也不得用 `no rollout found` 推断 Side 消失。页面票据、Agent 或 app-server 已失效时仍无法补回此前未收到的 ephemeral 历史。
